...
Validate the contract's semantics, not the `interface` keyword. Confirm that representative implementations can honor the same documented behavior without consumer downcasts, concrete-type checks, unsupported members, or implementation-specific configuration. For a public or reusable abstraction, inspect several implementations and consuming APIs; consider shared contract tests.
...
Do not preserve an internal interface solely because tests mock it. If behavior-preserving refactors break tests because they assert internal calls, treat that as test coupling: test through a stable behavior boundary and place doubles at slow, nondeterministic, or external effects.
...
Evaluate CQRS, request/handler organization, vertical slicing, and mediator dispatch as separate choices. Splitting reads from writes does not require MediatR, and using MediatR does not prove that CQRS or vertical slices add value.
...
Keep mediator dispatch at a clear application or host edge. Question handlers or domain services that dispatch to other internal handlers: nested dispatch hides the call graph unless independent dispatch or notification semantics are the actual job.

Do not count a pipeline behavior as a job merely because it exists. Compare host-only concerns with ASP.NET Core middleware or endpoint filters; keep mediator behaviors when the policy belongs to use-case dispatch, varies by request type, or must work across hosts.
...
Prefer platform documentation for framework facts. Treat practitioner sources as experience-backed heuristics and apply them only after repository evidence.
...
- [Framework Design Guidelines: abstractions](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/abstractions-abstract-types-and-interfaces): give reusable-library heuristics for semantic contracts, multiple implementations, consuming APIs, and contract tests; apply narrowly because the page warns that some guidance is dated.
...
- [Testing with `FakeTimeProvider`](https://learn.microsoft.com/en-us/dotnet/core/extensions/timeprovider-testing): demonstrate the built-in controllable time seam and deterministic testing.
...
- [Filters in Minimal API apps](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/minimal-apis/min-api-filters?view=aspnetcore-10.0): document the host-native before/after pipeline for validation, logging, and endpoint concerns.
- [MediatR repository](https://github.com/LuckyPennySoftware/MediatR): define the library's current capabilities as in-process dispatch and pipeline behaviors, not as proof that an application needs CQRS or vertical slices.
- [Interfaces are not abstractions](https://blog.ploeh.dk/2010/12/02/Interfacesarenotabstractions/): provide Mark Seemann's .NET/DI heuristics for substitutability failures and leaky contracts; treat them as design arguments rather than empirical rules.
- [TDD: Where Did It All Go Wrong?](https://www.infoq.com/presentations/tdd-original/): provide Ian Cooper's experience-based warning that mock-heavy internal tests can couple production design to implementation details.
- [Vertical Slice Architecture](https://www.jimmybogard.com/vertical-slice-architecture/): provide Jimmy Bogard's original axis-of-change argument; use it as a repository-specific heuristic, not a universal architecture mandate.
- [MediatR author's usage boundary](https://www.jimmybogard.com/you-probably-dont-need-to-worry-about-mediatr/): describe dispatch at application edges in the author's architecture; use it as intended-usage context, not neutral evidence that the dependency is necessary.
